    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="100px">
        <el-form-item label="会员手机号:">
          <el-input-number v-model="formData.userID" clearable placeholder="请输入" />
        </el-form-item>
        <el-form-item label="选择商品:">
          <el-input-number v-model="formData.cardID" clearable placeholder="请输入" />
        </el-form-item>
        <el-form-item label="赠送数量:">
          <el-input-number v-model="formData.giftAmt" clearable placeholder="请输入" />
        </el-form-item>
        <el-form-item label="实收价格:">
          <el-input-number v-model="formData.payPrice" clearable placeholder="请输入" />
        </el-form-item>
...
import {
  createUserEntryCard,
  deleteUserEntryCard,
  deleteUserEntryCardByIds,
  updateUserEntryCard,
  findUserEntryCard,
  getUserEntryCardList
} from '@/api/userEntryCard'
...
const getTableData = async() => {
  const table = await getUserEntryCardList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteUserEntryCardByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserEntryCardFunc = async(row) => {
    const res = await findUserEntryCard({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.reuserEntryCard
...
// 删除行
const deleteUserEntryCardFunc = async (row) => {
    const res = await deleteUserEntryCard({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createUserEntryCard(formData.value)
                  break
                case 'update':
                  res = await updateUserEntryCard(formData.value)
                  break
                default:
                  res = await createUserEntryCard(formData.value)
                  break
